...
import {
    Heart, MessageCircle, Share2, Sparkles, MoreHorizontal,
    Image as ImageIcon, Send, X, Mic, MicOff, Loader2, ExternalLink, Trash2
} from "lucide-react";
...
function CommentItem({
    ...
    return (
        <div className="flex gap-3 text-sm mb-3 group/comment animate-in fade-in slide-in-from-top-1">
            ...
            <div className="flex-1 space-y-1">
                <div className="bg-muted/50 rounded-2xl px-4 py-2">
                    ...
                    {isEditing ? (
                        <div className="flex gap-2">
                            <Input value={editContent} onChange={e => setEditContent(e.target.value)} className="h-8 text-sm" autoFocus />
                ...
                {comment.mediaUrl && (
                    <div className="mt-1 rounded-xl overflow-hidden max-w-sm border border-border">
                ...
                <div className="flex items-center gap-4 px-2">
                    <button
                        onClick={handleLike}
                        className={cn("text-xs font-medium hover:underline flex items-center gap-1", isLiked ? "text-pink-600" : "text-muted-foreground")}
                    >
                        {isLiked ? "Liked" : "Like"}
                        {likes.length > 0 && <span className="bg-muted px-1 rounded-full">{likes.length}</span>}
                    </button>

                    {isAuthor && (
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <button className="text-xs font-medium text-muted-foreground hover:underline">More</button>
                            </DropdownMenuTrigger>
...
export function PostCard({ post, currentUserId }: { post: any, currentUserId?: string }) {
    ...
    const onReaction = async (type: ReactionType) => {
        ...
        setAllReactions((prev: any) => {
            const next = { ...prev };
            if (isRemoving && currentUserId) delete next[currentUserId];
            else if (currentUserId) next[currentUserId] = type;
            return next;
        });
    ...
    // -- Render --
    return (
        <Card className="mb-4 overflow-hidden border-none shadow-sm bg-card/50 hover:bg-card transition-colors">
                    ...
                    <AlertDialog>
                        ...
                        <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                                ...
                                <AlertDialogAction onClick={onDeletePost} className="bg-destructive hover:bg-destructive/90">
            ...
            <CardContent className="px-4 py-2">
                ...
                {post.mediaUrls?.map((url: string, idx: number) => (
                    <div key={idx} className="mt-3 rounded-xl overflow-hidden border border-border bg-black/5">
                        {isUrlVideo(url) ? (
            ...
            <CardFooter className="flex-col !items-stretch px-2 py-1 mx-2 mt-1 border-t border-border/50">
                {/* Stats Row */}
                    ...
                    <div className="flex justify-between items-center text-xs text-muted-foreground w-full mb-2 px-1 pt-2">
                        <div className="flex items-center gap-1">
                            <div className="flex -space-x-1">
                                {Object.values(allReactions).slice(0, 3).map((r: any, i) => (
                                    <span key={i} className="bg-background rounded-full p-0.5" title={getReactionLabel(r)}>{getReactionIcon(r)}</span>
                                ))}
                            </div>
                            {Object.keys(allReactions).length > 0 && <span>{Object.keys(allReactions).length}</span>}
                        </div>
                ...
                <div className="flex justify-between items-center w-full mb-1">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="ghost" className={cn("flex-1 gap-2 hover:bg-pink-50 dark:hover:bg-pink-950/20", currentMyReaction ? "text-pink-600" : "text-muted-foreground")}>
                                <span className={cn("text-xl transition-transform", currentMyReaction && "scale-110")}>{getReactionIcon(currentMyReaction)}</span>
                                <span>{currentMyReaction ? getReactionLabel(currentMyReaction) : "Like"}</span>
                            </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="start" className="flex gap-1 p-2">
                            {REACTIONS.map(r => (
                                <DropdownMenuItem key={r.type} onClick={() => onReaction(r.type)} className="text-2xl cursor-pointer hover:scale-125 transition-transform" title={r.label}>
                                    {r.emoji}
                                </DropdownMenuItem>
                            ))}
                        </DropdownMenuContent>
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="ghost" className="flex-1 gap-2 text-muted-foreground hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-950/20">
                ...
                {/* Comments Section */}
                {showComments && (
                    <div className="w-full mt-4 space-y-4 animate-in slide-in-from-top-2 duration-300">
                        {/* Input Area */}
                        <div className="flex gap-2 items-start">
                            ...
                            <div className="flex-1 relative">
                                ...
                                <div className="absolute right-1 top-1 flex items-center gap-1">
                                    {/* Magic Suggestion */}
                                    <Button size="icon" variant="ghost" className="h-7 w-7 text-purple-500 hover:bg-purple-100" onClick={async () => {
                                        try {
                        ...
                        {/* Upload Indicator */}
                        {isUploading && <div className="text-xs text-muted-foreground flex items-center gap-2 px-10"><Loader2 className="w-3 h-3 animate-spin" /> Uploading media...</div>}
                        {commentMediaUrl && <div className="text-xs text-green-600 px-10">Media attached</div>}
