    ...
    return (
        <MainLayout className="max-w-7xl w-full h-[calc(100vh-theme(spacing.32))]">
            <div className="flex gap-6 h-full">
                {/* Sidebar List */}
                <div className={`w-full md:w-80 flex flex-col bg-card rounded-lg border border-border dark:border-white/5 overflow-hidden shadow-sm ${activeChatId ? 'hidden md:flex' : 'flex'}`}>
                    <div className="p-4 border-b border-border dark:border-white/5 flex items-center justify-between">
                ...
                <div className={`flex-1 ${!activeChatId ? 'hidden md:flex' : 'flex'} flex-col h-full`}>
                    {activeSession ? (
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center bg-card/50 dark:bg-card/50 rounded-lg border border-dashed border-border dark:border-white/10 p-8 text-center text-muted-foreground animate-in fade-in zoom-in-95 duration-500 h-full">
